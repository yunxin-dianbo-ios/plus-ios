    ...
    if (self) {
        ...
        switch (_firstMessageBody.type) {
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                // 自己发送的Gif消息需要特别的初始化
                NSInteger currentUID = [[[NSUserDefaults standardUserDefaults] objectForKey:@"TSCurrentUserInfoModel.uid"] integerValue];

                if (imageData.length) {
                    if ([message.from integerValue] == currentUID) {
                        if (message.ext != nil && [message.ext[@"mimeType"] isEqualToString:@"image/gif"]) {
                            // gif
                            self.image = [UIImage sd_tz_animatedGIFWithData:imageData];
                        } else {

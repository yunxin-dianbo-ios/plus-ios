...
- (void)setShowRefreshHeader:(BOOL)showRefreshHeader
{
    if (_showRefreshHeader != showRefreshHeader) {
        ...
        if (_showRefreshHeader) {
            ...
            MJRefreshGifHeader *headerR = [MJRefreshGifHeader headerWithRefreshingBlock:^{
                [weakSelf tableViewDidTriggerHeaderRefresh];
            }];
            NSMutableArray *headerImages = [NSMutableArray array];
            for (int i = 0; i <= 14; i++) {
                UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"IMG_refresh_icon_header_000%d",i]];
                [headerImages addObject:image];
            }
            headerR.stateLabel.hidden = YES;
            headerR.lastUpdatedTimeLabel.hidden = YES;
            [headerR setImages:@[headerImages[0]]forState:MJRefreshStateIdle];
            [headerR setImages:headerImages forState:MJRefreshStateRefreshing];

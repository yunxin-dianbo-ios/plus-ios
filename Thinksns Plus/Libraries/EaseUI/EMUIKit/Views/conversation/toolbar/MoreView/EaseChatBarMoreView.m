...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    /// 第二排三个按钮布局左右边距
    CGFloat secondInsets = (self.frame.size.width - 2 * CHAT_BUTTON_SIZE.width - insets) / 2;
...
//                                title:@"相册"];
    _photoButton = [self btnWithImage:[UIImage imageNamed:@"ico_chat_picture"]
                     highlightedImage:[UIImage imageNamed:@"ico_chat_picture"]
                                title:@"图片"];
...
//                                   title:@"位置"];
    _locationButton = [self btnWithImage:[UIImage imageNamed:@"ico_chat_location"]
                        highlightedImage:[UIImage imageNamed:@"ico_chat_location"]
                                   title:@"位置"];
    _locationButton.accessibilityIdentifier = @"location";
    [_locationButton setFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE.width * 2, 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height)];//insets * 2 + CHAT_BUTTON_SIZE.width, 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height
...
//                                  title:@"拍照"];
    _takePicButton = [self btnWithImage:[UIImage imageNamed:@"ico_chat_takephoto"]
                       highlightedImage:[UIImage imageNamed:@"ico_chat_takephoto"]
                                  title:@"拍照"];
    [_takePicButton setFrame:CGRectMake(insets * 2 + CHAT_BUTTON_SIZE.width, 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height)];//insets * 3 + CHAT_BUTTON_SIZE.width * 2, 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height
    [_takePicButton addTarget:self action:@selector(takePicAction) forControlEvents:UIControlEventTouchUpInside];
    _takePicButton.tag = MOREVIEW_BUTTON_TAG + 2;
    _maxIndex = 2;
    [_scrollview addSubview:_takePicButton];
    
    _videoButton = [self btnWithImage:[UIImage imageNamed:@"ico_chat_video"]
                       highlightedImage:[UIImage imageNamed:@"ico_chat_video"]
                                  title:@"视频"];
    [_videoButton setFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE.width * 2, 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height)];
    [_videoButton addTarget:self action:@selector(takePicAction) forControlEvents:UIControlEventTouchUpInside];
...
//                                        title:@"语音"];
        _audioCallButton = [self btnWithImage:[UIImage imageNamed:@"ico_chat_voicecall"]
                             highlightedImage:[UIImage imageNamed:@"ico_chat_voicecall"]
                                        title:@"语音通话"];
        [_audioCallButton setFrame:CGRectMake(secondInsets, 10 * 2 + CHAT_BUTTON_SIZE.height + 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height)];
...
//                                        title:@"视频"];
        _videoCallButton = [self btnWithImage:[UIImage imageNamed:@"ico_chat_videocall"]
                             highlightedImage:[UIImage imageNamed:@"ico_chat_videocall"]
                                        title:@"视频通话"];
        [_videoCallButton setFrame:CGRectMake(secondInsets + CHAT_BUTTON_SIZE.width + insets, 10 * 2 + CHAT_BUTTON_SIZE.height + 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height)];
    ...
    {
        frame.size.height = 80 + 15;
    }
    self.frame = frame;
    _scrollview.frame = CGRectMake(0, 15, CGRectGetWidth(frame), CGRectGetHeight(frame));
...
- (UIButton *)btnWithImage:(UIImage *)aImage highlightedImage:(UIImage *)aHighLightedImage title:(NSString *)aTitle {
    ...
    btn.titleEdgeInsets = UIEdgeInsetsMake(14+15/2.0, -60+10, -20, 0);
    return btn;
}

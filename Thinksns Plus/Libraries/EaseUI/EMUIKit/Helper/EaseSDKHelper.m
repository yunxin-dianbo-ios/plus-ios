...
// 判断是不是长图
+ (BOOL)isLongImage:(CGSize )imageSize {
    float imageWHRatio = imageSize.height / imageSize.width;
    float screenWHRatio = [UIScreen mainScreen].bounds.size.height / [UIScreen mainScreen].bounds.size.width;
    return (imageWHRatio / screenWHRatio) > 3;
}
// 返回聊天详情图片规则的尺寸
+ (CGSize)getChatImageRegularSize:(CGSize)imageSize {
    CGSize regularSize;
    CGSize originalSize = imageSize;
    CGSize screenSize = [UIScreen mainScreen].bounds.size;
    CGFloat maxImageWidth = screenSize.width * (4 / 15.0);
    CGFloat maxImageHeight = screenSize.height * (4 / 15.0) * (16 / 9.0);
    CGFloat imageWHRatio = originalSize.width / originalSize.height;
    if (originalSize.width < 5 || originalSize.height < 5) {
        regularSize.width = maxImageWidth * 0.5;
        regularSize.height = maxImageHeight * 0.5;
    } else {
        // 判断是否是长图
        BOOL islongImage = [EaseSDKHelper isLongImage:originalSize];
        if (islongImage == YES) {
            // 长图
            if (originalSize.width < originalSize.height) {
                regularSize.width = maxImageWidth;
                regularSize.height = maxImageWidth * 2;
            } else {
                regularSize.width = maxImageWidth;
                regularSize.height = maxImageWidth / 2;
            }
            ...
            if (originalSize.width > originalSize.height) {
                if (originalSize.width > maxImageWidth) {
                    regularSize.width = maxImageWidth;
                    regularSize.height = regularSize.width / imageWHRatio;
                } else {
                    // 没有超过限制就保留原始样式
                    regularSize = originalSize;
                }
                ...
                if (originalSize.height > maxImageHeight) {
                    regularSize.height = maxImageHeight;
                    regularSize.width = regularSize.height * imageWHRatio;
                    if (regularSize.width > maxImageWidth) {
                        regularSize.width = maxImageWidth;
                        regularSize.height = regularSize.width / imageWHRatio;
                    }
